...
const char info[] = "Yamaha Power Valve System - NB YPVS Driver\nWARNING "
                    "- this device has a development version software\n"
...
atresult_t at_rpm_service(uint8_t inout, char* params) {
  ...
  if (inout == param) {
    ...
    char answerBuff[20];
    snprintf(answerBuff, sizeof(answerBuff), "AT_RPM=%lu\n", getEngineRPM());
    sendAnswer(&atCommandService, answerBuff);
  }
  return AT_OK;
...
atresult_t at_srpm_service(uint8_t inout, char* params) {
  if (inout == ask) {
    char answerBuf[20];
    snprintf(answerBuf, sizeof(answerBuf), "%d", getRPMState());
